...
import type { HealthReport, FixAction, Conflict } from "../utils/types.js";
...
export async function generateHealthReport(options: HealthReportOptions): Promise<HealthReport> {
...
function generateFixes(conflicts: Conflict[]): FixAction[] {
...
function generateTriggerRefinement(trigger: string, skills: string[]): string {
  return skills
    .map((skill) => `  ${skill}: "${trigger} for ${skill.replace(/-/g, " ")}"`)
    .join("\n");
}

function generatePriorityPatch(skills: string[]): string {
  return skills
