...
import type { SkillManifest, SkillRecord, ValidationResult } from "./types.js";
...
export async function parseSkillFile(filepath: string): Promise<SkillRecord> {
  ...
  try {
    manifest = parseYaml(parts[1]) as SkillManifest;
  } catch (e) {
    return createErrorRecord(filepath, content, stats, `YAML parse error: ${(e as Error).message}`);
  }
...
export function validateSkill(record: SkillRecord, content: string): ValidationResult {
