...
function getSkillsDir(config: Config): string {
  const fromConfig = config.skillsDir as string | undefined;
...
export default definePluginEntry({
  ...
  description:
    "Discovers, validates, audits, and resolves conflicts across all workspace skills. " +
  ...
  register(api) {
    ...
    api.registerTool({
      ...
      label: "Audit Skills",
      description:
        "Run a comprehensive audit of all skills in the workspace. Returns inventory, validation results, and missing dependencies.",
      parameters: Type.Object({
        skillsDir: Type.Optional(Type.String({ description: "Path to skills directory" })),
      }),
      async execute(_id: string, params: { skillsDir?: string }) {
        ...
        if (missing.length) {
          ...
          for (const dep of missing) lines.push(`- **${dep.binary}** (required by: ${dep.requiredBy.join(", ")})`);
        }
        const failed = report.validationResults.filter((r) => !r.valid);
        if (failed.length) {
          lines.push("", "## ❌ Validation Failures");
          for (const v of failed) lines.push(`- **${v.name}**: ${v.errors.join("; ")}`);
        }
    ...
    api.registerTool({
      ...
      description:
        "Detect conflicts between skills: trigger overlaps, similar triggers, description clashes, tool competition, and name collisions.",
      parameters: Type.Object({
        skillsDir: Type.Optional(Type.String({ description: "Path to skills directory" })),
        similarityThreshold: Type.Optional(
          Type.Number({ description: "Similarity threshold (0.0-1.0)", minimum: 0, maximum: 1 })
        ),
      }),
      async execute(_id: string, params: { skillsDir?: string; similarityThreshold?: number }) {
        const dir = params.skillsDir || getSkillsDir(config);
        ...
        const buckets = { CRITICAL: [] as typeof conflicts, HIGH: [] as typeof conflicts, MEDIUM: [] as typeof conflicts, LOW: [] as typeof conflicts };
        for (const c of conflicts) buckets[c.severity].push(c);
        for (const sev of ["CRITICAL", "HIGH", "MEDIUM", "LOW"] as const) {
          ...
          for (const c of list) {
            ...
            if (c.triggerA && c.triggerB) lines.push(`- **Triggers:** "${c.triggerA}" ↔ "${c.triggerB}" (${c.similarity})`);
            if (c.sharedTools) lines.push(`- **Shared Tools:** ${c.sharedTools.join(", ")}`);
    ...
    api.registerTool({
      ...
      description:
        "Validate all SKILL.md files in the workspace. Checks YAML syntax, required fields, trigger quality, and security (no hardcoded secrets).",
      parameters: Type.Object({
        skillsDir: Type.Optional(Type.String({ description: "Path to skills directory" })),
        strictMode: Type.Optional(Type.Boolean({ description: "Fail on warnings too" })),
      }),
      async execute(_id: string, params: { skillsDir?: string; strictMode?: boolean }) {
        const dir = params.skillsDir || getSkillsDir(config);
        ...
        for (const result of report.results) {
          ...
          for (const info of result.info) lines.push(`- ℹ️ ${info}`);
          lines.push("");
        }
        return { details: "", content: [{ type: "text", text: lines.join("\n") }] };
      },
    ...
    api.registerTool({
      ...
      label: "Health Report",
      description:
        "Generate a comprehensive health report combining audit, validation, and conflict detection. Returns overall health and recommended fixes.",
      parameters: Type.Object({
        skillsDir: Type.Optional(Type.String({ description: "Path to skills directory" })),
        similarityThreshold: Type.Optional(Type.Number({ minimum: 0, maximum: 1 })),
        strictMode: Type.Optional(Type.Boolean()),
      }),
      async execute(_id: string, params: { skillsDir?: string; similarityThreshold?: number; strictMode?: boolean }) {
        const dir = params.skillsDir || getSkillsDir(config);
        const threshold = params.similarityThreshold ?? num(config, "similarityThreshold", 0.6);
        const strict = params.strictMode ?? bool(config, "strictMode");
        ...
        if (report.conflicts.length) {
          ...
          for (const c of report.conflicts) lines.push(`- **[${c.severity}]** ${c.type}: ${c.message}`);
          lines.push("");
        }
    ...
    api.registerTool({
      ...
      description:
        "Apply automatic fixes to skill issues. Supports trigger refinement, priority assignment, and safe manifest corrections. Always backs up before modifying. Dry-run by default.",
      parameters: Type.Object({
        skillsDir: Type.Optional(Type.String({ description: "Path to skills directory" })),
      ...
      async execute(_id: string, params: { skillsDir?: string; fixType: string; target: string; dryRun?: boolean }) {
        const dir = params.skillsDir || getSkillsDir(config);
        const isDry = params.dryRun ?? true;
        const result = await applyFix({
          fix: {
            type: params.fixType as "REFINE_TRIGGER" | "ADD_PRIORITY" | "RENAME_SKILL" | "MERGE_SKILLS",
